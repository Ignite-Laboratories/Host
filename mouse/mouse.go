...
var Coordinates = temporal.Observer(core.Impulse, when.Frequency(&SampleRate), true, Sample)
...
func Reaction(engine *core.Engine, frequency *float64, onChange temporal.Change[std.XY[int]]) *temporal.Dimension[std.XY[int], any] {
	return temporal.Reaction[std.XY[int]](engine, when.Frequency(frequency), false, Sample, onChange)
}

// Sample gets the current mouse coordinates, or nil if unable to do so.
func Sample() *std.XY[int] {
	defer func() {
		if r := recover(); r != nil {
